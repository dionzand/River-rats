...
(function (root) {
  ...
  /* Every game gets its own engine. The state lives in the closure rather than
     on a shared object, so a server can hold several rooms at once: two games
     both parked waiting for someone to tap would otherwise take turns
  ...
  /* One game per page, so the module itself is an engine - and `create` makes
     another for anyone holding more than one at a time. */
